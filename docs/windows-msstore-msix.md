...
이 서명 비용을 0으로 만드는 우회로가 몇 가지 있는데(예: 오픈소스 프로젝트용 무료 서명 프로그램 등), 그 중 가장 확실한 게 **MS Store 배포**다. Store에 올리면 Microsoft가 자기 인증서로 **재서명**해 주고, Store를 통해 설치된 앱은 SmartScreen 경고가 없다. 인증서를 직접 사거나 갱신·관리할 필요가 전혀 없다 — 서명을 Microsoft에게 위임하는 셈이다. (대신 등록비 1회와 심사라는 비용을 치른다. 심사 경험은 글 끝에.)
...
Partner Center 대시보드의 앱 → 새 제출(submission)에 unsigned `.msix`를 올린다. 통과하면 게시된다. 다만 "수십 분~수 시간"은 이상적인 경우고, 실제 첫 제출은 더 걸릴 수 있다 — 아래 경험 참고.

## 8. 경험 공유 — 심사는 생각보다 오래, 탈락은 기본값

실제 앱 하나를 Store에 올리며 겪은 기록이다.

- **첫 심사 결과까지 약 4일.** 문서엔 보통 수 시간이라지만, 신규 게시자의 첫 앱은 수동 검토가 붙는지 4일쯤 걸렸다.
- **첫 제출은 탈락.** 대신 **사유를 아주 구체적으로 알려준다** — 어떤 정책 항목을, 어느 화면/동작에서, 어떻게 위반했는지까지. 막연한 거절이 아니라 그대로 고칠 수 있는 피드백이다.
- **수정·재심사 후에도 하루 만에 또 탈락.** 재제출은 첫 심사보다 빨라서 하루 안팎으로 결과가 나오지만, 한 번에 다 통과하긴 어렵다. **탈락을 4회쯤 반복**하며 사유를 하나씩 잡고 나서야 통과했다.
- **통과 이후 업데이트는 하루 정도.** 일단 한 번 승인되면 그 다음부터 버전 업데이트 심사는 하루 안팎으로 빨라진다.

요점: **첫 통과까지는 일정을 크게 잡아라.** 탈락은 거의 기본값에 가깝고, 사유가 상세하니 좌절할 필요 없이 반복하면 된다. 출시 데드라인이 있다면 최소 1~2주 버퍼를 두는 게 안전하다.
...
| 심사 | Partner Center | 첫 통과까지 1~2주 버퍼, 탈락은 기본값 |
